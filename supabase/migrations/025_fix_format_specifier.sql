-- ============================================================================
-- Migration 025: format() 서식 문자 버그 수정
-- 문제: PostgreSQL format()은 %.1f, %.0f 미지원 → "unrecognized format() type specifier"
-- 해결: ROUND()로 미리 계산 후 %s 서식 사용
-- ============================================================================
...
BEGIN
  ...
  scored AS (
    SELECT
      ...
      jsonb_exists(industry_rates,  COALESCE(t.industry_code,''))            AS has_ind,
      jsonb_exists(agency_rates,    COALESCE(t.demand_agency_name,''))       AS has_agn,
      jsonb_exists(competition_avg, COALESCE(t.industry_code,''))            AS has_comp,
  ...
  scored_full AS (
    SELECT s.*,
      ...
      CASE
        WHEN jsonb_exists(industry_rates,  COALESCE(s.industry_code,''))
         AND jsonb_exists(agency_rates,    COALESCE(s.demand_agency_name,''))
         AND jsonb_exists(competition_avg, COALESCE(s.industry_code,'')) THEN 'real'
        WHEN jsonb_exists(industry_rates,  COALESCE(s.industry_code,''))
          OR jsonb_exists(agency_rates,    COALESCE(s.demand_agency_name,'')) THEN 'partial'
        ELSE 'insufficient'
      END AS data_quality
    FROM scored s
  ),
    ...
    COALESCE((
      ...
      FROM (
        SELECT s.id, s.source_tender_id, s.title,
               ...
               -- format() %.1f 사용 불가 → ROUND + 문자열 연결로 교체
               CASE
                 WHEN s.has_comp THEN
                   '평균 경쟁 업체 수 ' || ROUND(s.avg_bidders_raw, 1)::text || '개 (데이터 기반)'
        ...
        SELECT s.id, s.source_tender_id, s.title,
               ...
               CASE
                 WHEN s.budget_amount IS NOT NULL THEN
                   ...
                   END
                   || ', 기회 점수 ' || ROUND(s.win_probability)::text || '점'
                 ELSE '고예산 공고입니다'
               END AS reason,
               s.data_quality
